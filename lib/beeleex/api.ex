defmodule Beeleex.Api do
  ...
  @doc """
  Get the Business Unit identified by `id`.

  This is a server-to-server call authenticated with the configured `secure-key`
  and `bu-id` headers.
  """
  @spec get_business_unit(integer() | String.t()) ::
          {:ok, Beeleex.BusinessUnit.t()} | {:error, String.t()}
  def get_business_unit(id) do
    case ExGeeks.Helpers.endpoint_post_callback(
           url(),
           %{
             query: """
             query getBusinessUnit($id: Int!) {
      ...
      %{"data" => %{"getBusinessUnit" => business_unit}} when not is_nil(business_unit) ->
        {:ok, parse_business_unit(business_unit)}
  ...
  @doc """
  Update the Business Unit identified by `id` with the given `BusinessUnitInput` map.

  `business_unit_input` must match Beelee's GraphQL input shape with camelCase
  keys, e.g. `%{name:, cycle:, startCycleDate:, tokenValidationUrl:,
  verifyCardAttachment:, webhookUrl:}`.
  """
  @spec edit_business_unit(integer() | String.t(), map()) ::
          {:ok, Beeleex.BusinessUnit.t()} | {:error, String.t()}
  def edit_business_unit(id, business_unit_input) do
    case ExGeeks.Helpers.endpoint_post_callback(
           url(),
           %{
             query: """
             mutation editBusinessUnit($businessUnit: BusinessUnitInput!, $id: Int!) {
               editBusinessUnit(businessUnit: $businessUnit, id: $id) {
                 #{business_unit_fields()}
               }
             }
             """,
             variables: %{businessUnit: business_unit_input, id: to_int(id)}
           },
           headers()
         ) do
      %{"data" => %{"editBusinessUnit" => business_unit}} when not is_nil(business_unit) ->
        {:ok, parse_business_unit(business_unit)}
  ...
  @doc """
  Run the next scheduled cycle for the Business Unit identified by `id`.

  This is a server-to-server call authenticated with the configured `secure-key`
  and `bu-id` headers.
  """
  @spec run_next_scheduled_business_unit_cycle(integer() | String.t()) ::
          {:ok, Beeleex.BusinessUnit.t()} | {:error, String.t()}
  def run_next_scheduled_business_unit_cycle(id) do
    case ExGeeks.Helpers.endpoint_post_callback(
           url(),
           %{
             query: """
             mutation runNextScheduledBusinessUnitCycle($id: Int!) {
               runNextScheduledBusinessUnitCycle(id: $id) {
                 #{business_unit_fields()}
               }
             }
             """,
      ...
      %{"data" => %{"runNextScheduledBusinessUnitCycle" => business_unit}}
      when not is_nil(business_unit) ->
        {:ok, parse_business_unit(business_unit)}
  ...
  defp parse_business_unit(business_unit) do
    business_unit
    |> ExGeeks.Helpers.atomize_keys(transformer: &Macro.underscore/1)
    |> then(&struct(Beeleex.BusinessUnit, &1))
  end
