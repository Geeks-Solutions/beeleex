defmodule Beeleex.Webhook do
  @moduledoc """
  Secures a Beelee Event from webhook's payload if signature is valid.
  """
  ...
  @doc """
  Verify webhook payload and return a valid Beelee event.

  `payload` is the raw, unparsed content body sent by Beelee, which can be
  ...
  `signature` is the value of `Beelee-Signature` header, which can be fetched
  with `Plug.Conn.get_req_header/2`.

  `secret` is your webhook endpoint's secret from the Beelee Dashboard.
  ...
  ## Example
      ...
      case Beeleex.Webhook.construct_event(payload, signature, secret) do
        {:ok, %Beeleex.Event{} = event} ->
          # Return 200 to Beelee and handle event
